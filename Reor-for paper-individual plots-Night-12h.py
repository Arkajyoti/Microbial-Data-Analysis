...
pths='P:/HA452_Motility/24h_R2/Analysed/24hrs R1+R2/R2/Reor trial/Nyt/'
...
########################################################
def sfit(data,thtt,sgn,guess_amplitude) :
...
#N = 1000 # number of data points
    ...
    plt.plot(np.degrees(t),data,'go',label='original')
    ...
    plt.ylabel('Omega (rad/sec)',fontsize=19,fontweight=5)
    plt.xlabel('Theta (degrees)',fontsize=19,fontweight=5)
    plt.gcf().set_size_inches(6.5,4)
    #plt.figure().add_axes([0,0,1,1]) 
    plt.ylim(-.1,.1)
    plt.yticks([-.1,-.05,0,.05,.1])
    ...
    plt.savefig(pths+str(sgn)+'F_'+'w_'+str(round(max(data_fit),2))+'.jpg',bbox='tight',
            pad_inches=0.0)
...
avth,avom=avm(sttf),avm(soof)
avom[-1]=0
#avom[1]=.035

sfit(avom,avth,sgnz,.05)
